...
func chanDemo(){
	...
	for i,worker := range workers{
		worker.in <- 'a'+i
		 //<-workers[i].done  //由于 channel是发阻塞的，也就是非缓冲channel 接收到一个信息，必须被消费掉。
	}


	// 执行到这里，done中已经有一个数据待其他 active go routine消费，但是这时又往 同一个go routine  [go doWorker 这个 routine]的done中写数据，这时候发生dead lock
	// 解决方法，用 go func() {done <-true}() ,这样，每次都起一个新的go routine来进行channel接收，每次发生堵塞的都是不同的 go routine，就不会发生dead lock
	for i,worker := range workers{
		worker.in <- 'A'+i
		//<-workers[i].done
	}
...
// 不是发一个任务等结束、而是将所有任务全部发出去，然后等待全部结束后开始退出
 // 所有channel发 都是block操作，必须要另一端有消费
